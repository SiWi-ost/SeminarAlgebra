...
global colorrotation;
colorrotation = pi;
colorrotation = 0.77 * pi;
...
function retval = hue(w)
	global colorrotation;
	global colormultiplier;
	phi0 = colormultiplier * arg(f(w));
	retval = 360 * (phi0 + colorrotation) / (2 * pi);
end
...
function quadrat(fn, z)
	...
	phi0 = arg(f(z +     hx/3 +     i * hy/3));
	phi1 = arg(f(z + 2 * hx/3 + 2 * i * hy/3));
	fprintf(fn, "triangle {\n\t");
	punkt(fn, p00);
	fprintf(fn, ",\n\t");
	punkt(fn, p10);
	...
	fprintf(fn, "\n\tpigment {\n\t\t color CHSV2RGB(rgb<%.3f, %.2f, %.2f>)\n\t}",
		hue(z + hx + i * hy), saturation, value);
	fprintf(fn, "\n");
%	fprintf(fn, "\tfinish {\n\t\tmetallic\n\t\tspecular 0.99\n");
%	fprintf(fn, "\t}\n");
	fprintf(fn, "}\n");
	...
	punkt(fn, p01);
	fprintf(fn, "\n\tpigment {\n\t\t color CHSV2RGB(rgb<%.3f, %.2f, %.2f>)\n\t}",
		hue(z + 2 * (hx + i * hy)), saturation, value);
	fprintf(fn, "\n");
%	fprintf(fn, "\tfinish {\n\t\tmetallic\n\t\tspecular 0.99\n");
%	fprintf(fn, "\t}\n");
	fprintf(fn, "}\n");
